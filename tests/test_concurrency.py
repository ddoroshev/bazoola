...
def test_concurrent_seqnum_generation_threads(db):
    ...
    def insert_worker(worker_id):
        try:
            for i in range(10):
                result = db.insert("a", {"text": f"w{worker_id}_{i}"})
...
def process_worker(worker_id) -> list[int] | str:
    ...
    try:
        for i in range(100):
            result = db.insert("a", {"text": f"p{worker_id}_{i}"})
            results.append(result["id"])
        return results
    except Exception as e:
        return str(e)
    ...
    with ProcessPoolExecutor(max_workers=10) as executor:
        ...
        for future in futures:
            ...
            if isinstance(worker_results, list):
                ids.extend(worker_results)
            else:
                errors.append(worker_results)
...
def test_concurrent_delete_insert_race_condition(db):
    ...
    initial_ids = []
    for i in range(20):
        result = db.insert("a", {"text": f"r_{i}"})
    ...
    def delete_worker():
        try:
            for i in range(0, 20, 2):
                db.delete_by_id("a", initial_ids[i])
    ...
    def insert_worker():
        try:
            for i in range(10):
                result = db.insert("a", {"text": f"n_{i}"})
        ...
        try:
            for i in range(5):
                ...
                result = db.update_by_id(
                    "c", record_id, {"varchar": f"w{worker_id}_{i}", "int": new_value}
                )
...
def stress_worker(worker_id):
    ...
    try:
        for i in range(20):
            if i % 3 == 0:
                # Insert
                result = db.insert("a", {"text": f"{worker_id}_{i}"})
                ...
                if insert_ops:
                    ...
                    db.update_by_id("a", target_id, {"text": f"u{worker_id}_{i}"})
